document.addEventListener('DOMContentLoaded', () => {
	...
	const dgnlMajor = document.getElementById('dgnlMajor');

	// Load majors from API
	if (dgnlMajor) {
		loadMajorsFromApi(dgnlMajor);
	}
	...
	// Load VSAT majors from API
	const vsatMajor = document.getElementById('vsatMajor');
	if (vsatMajor) {
		loadVsatMajorsFromApi(vsatMajor);
	}
...
async function loadVsatMajorsFromApi(selectElement) {
	// VSAT-specific data for known majors (code -> {c1, c2, threshold, admission})
	const vsatDataMap = {
		'7140114': { c1: 'B03', c2: 'C01', threshold: 20, admission: 25.5 },
		'7480103': { c1: 'D01', c2: 'A01', threshold: 19, admission: 24.6 },
		'7480201': { c1: 'A01', c2: 'D01', threshold: 18.5, admission: 24.0 }
	};
	// Default values for majors without specific VSAT data
	const defaultVsatData = { c1: 'B03', c2: 'C01', threshold: 20, admission: 25.5 };
	...
	try {
		...
		majors.forEach((major) => {
			const option = document.createElement('option');
			const vsatData = vsatDataMap[major.code] || defaultVsatData;

			option.value = major.code || '';
			option.textContent = major.label || `${major.code} - ${major.name}` || major.code || '';
			option.dataset.code = major.code || '';
			option.dataset.c1 = vsatData.c1;
			option.dataset.c2 = vsatData.c2;
			option.dataset.threshold = vsatData.threshold;
			option.dataset.admission = vsatData.admission;
